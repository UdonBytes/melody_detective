...
function App() {
  ...
  return <main>
    <header className="app-header"><div className="brand"><span className="brand-icon">♪</span><div><strong>Melody Detective</strong><small>Listen · Remember · Play</small></div></div><div className="header-actions"><div className="stat"><span>★</span><div><small>Score</small><strong>{score}</strong></div></div><div className="stat streak"><span>🔥</span><div><small>Streak</small><strong>{streak}</strong></div></div><button className="settings-button" onClick={() => setSettings(true)} aria-label="Open settings">⚙</button></div></header>
    <section className="game-card">
      <div className="game-head"><div><p className="eyebrow exercise-key"><span className="key-root">{tonic}</span> <span className="key-mode">{keyMode === 'major' ? 'Major' : 'Minor'}</span></p><h1>Play the melody back</h1></div><button className="listen play-melody-button" aria-label="Play melody" onClick={() => play()} disabled={playing || !audioReady}><Sound /><span className="play-melody-label">{!audioReady ? 'Loading piano…' : playing ? 'Playing…' : 'Play melody'}</span></button></div>
